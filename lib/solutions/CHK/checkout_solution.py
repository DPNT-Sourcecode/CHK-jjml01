...
def checkout(skus):
    ...
    priceDictTuple = priceDict.items()
    print(priceDictTuple)
    for item in skuList:
        if item in ['A', 'B', 'H', 'K', 'P', 'Q', 'V']:
            pass
        else:
            finalPriceNoCompound.append(itemCount[skuList.index(item)] * int(priceDictTuple[skuList.index(item)][1]))
